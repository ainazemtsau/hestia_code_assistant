...
from support import load_shadow_module, repo_temp_dir


class ShadowInstallLibTests(unittest.TestCase):
    def test_safe_rel_path_rejects_root_absolute_and_escape_paths(self) -> None:
        install_lib = load_shadow_module("shadow_install_lib", "tools/install_lib.py")
    ...
    def test_resolve_manifest_path_requires_manifest_inside_workflow_root(self) -> None:
        install_lib = load_shadow_module("shadow_install_lib_manifest", "tools/install_lib.py")
    ...
    def test_ensure_root_agents_block_preserves_client_content_and_updates_single_block(self) -> None:
        install_lib = load_shadow_module("shadow_install_lib", "tools/install_lib.py")

        with repo_temp_dir() as workspace:
            agents_path = workspace / "AGENTS.md"
            agents_path.write_text("# Client notes\n\nKeep this line.\n", encoding="utf-8")

            block_v1 = (
                f"{install_lib.BEGIN_MARKER}\nbootstrap v1\n{install_lib.END_MARKER}\n"
            )
            block_v2 = (
                f"{install_lib.BEGIN_MARKER}\nbootstrap v2\n{install_lib.END_MARKER}\n"
            )

            action_1 = install_lib.ensure_root_agents_block(agents_path, block_v1)
            action_2 = install_lib.ensure_root_agents_block(agents_path, block_v2)

            text = agents_path.read_text(encoding="utf-8")
            self.assertEqual("merged", action_1)
            self.assertEqual("updated", action_2)
            ...
            self.assertEqual(1, text.count(install_lib.BEGIN_MARKER))
            self.assertEqual(1, text.count(install_lib.END_MARKER))
