...
def _load_install_lib():
    ...
    spec = importlib.util.spec_from_file_location("csk_phase1_install_lib", module_path)
...
def install_client_workflow(workflow_root: Path, manifest_path: Path | None = None) -> dict[str, Any]:
...
def _build_parser() -> argparse.ArgumentParser:
    parser = argparse.ArgumentParser(description="Install the managed workflow base into the parent project.")
    parser.add_argument("--workflow-root", default=None, help="Workflow checkout root. Defaults to this script's repo root.")
...
def main() -> int:
    ...
    try:
        ...
        print(f"[csk-install] failed: {exc}")
        return 1

    print(f"[csk-install] project_root={summary['project_root']} assets={len(summary['installed_assets'])}")
